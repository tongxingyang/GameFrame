...
namespace GameFrame.Editor
{
    public class AssetBundlePanelConfig:ScriptableObject
    {
        ...
        public int[] Platform = new int[4] {0, 0, 0, 0};// ios,androis,mac,win
    }
    [Serializable]
    public class Version
    {
        ...
        public string GetAppPackageName()
        {
            ...
            return string.Format("{4}-ver{0:D2}.{1:D2}.{2:D2}-{3}", master, minor, revised,dstr,GameConfig.AppName);
        } 
    }
    [Serializable]
    ...
    public class AssetBundleBuildPanel:EditorWindow
    {
        ...
        private static string ScenePath = "Assets/Scene";//场景文件保存的位置
        private static AssetBundlePanelConfig _config;
        private ReorderableList _list;
        private Vector2 scrollPos = Vector2.zero;
        private static AssetBundleBuildPanel panel;
        //打包过滤的文件夹集合
        static List<string> filterDirList = new List<string>();
        //过滤的文件后缀
        ...
        public static  BuildTarget GetBuildTarget(int index)
        {
            if (index == 0)
            {
        ...
        static void BuildScene()
        {
            if (_config != null)
            {
                for (int j = 0; j < _config.Platform.Length; j++)
                {
                    if (_config.Platform[j] == 1)
                    {
                        string [] levels = Directory.GetFiles(ScenePath,"*.unity");
            
                        for (int i = 0; i < levels.Length; i++)
                        {
                            string[] temp = {levels[i]};
                            System.IO.FileInfo fileInfo = new  System.IO.FileInfo(levels[i]);
                            BuildPipeline.BuildPlayer(temp, Application.dataPath+"/"+fileInfo.Name+".unity3d",GetBuildTarget(j), BuildOptions.BuildAdditionalStreamedScenes);
                        }
...
//            AssetDatabase.RemoveUnusedAssetBundleNames();
        ...
        public static void ClearAssetBundleNames()
        {
            ...
            {
                if (names[i].IndexOf(Platform.AssetBundleExt, StringComparison.Ordinal) != -1)
                {
